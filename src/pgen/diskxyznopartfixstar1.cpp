...
void turner(FieldDiffusion *pfdif, MeshBlock *pmb, const AthenaArray<Real> &w,
...
//;  }
  ...
  if (MAGNETIC_FIELDS_ENABLED){
    ...
    eta_A0           = pin->GetReal("problem", "eta_ad");
    eta_O0           = pin->GetReal("problem", "eta_ohm");
    if (eta_A0 != 0 || eta_O0 != 0){
      ...
      grav_scale  = pin->GetReal("problem", "grav_scale");
      etaA_scale  = pin->GetReal("problem", "etaA_scale");
  ...
  if (nonidealfile != "None" && (eta_A0 != 0 || eta_O0 != 0)){
    ...
    logbetatable.NewAthenaArray(21);
    FILE *filenonideal;
    if ( (filenonideal=fopen(nonidealfile.c_str(),"r"))==NULL )
    {
      printf("Open non-ideal input file error\n");
      return;
    }
    int i, j, k, h;
    ...
    fclose(filenonideal);
    if(line) free(line);
    dlogt=logttable(1)-logttable(0);
    dlogr=logrhogtable(1)-logrhogtable(0);
    dlogz=logzetatable(1)-logzetatable(0);
    dlogb=logbetatable(1)-logbetatable(0);
  }
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin)
{
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      for(int i=is; i<=ie; i++) {
        Real alfven2 = (SQR(pfield->bcc(IB1,k,j,i))
...
void VelProfileCyl(MeshBlock *pmb, const Real rad, const Real phi, const Real z,
  ...
  if (pmb->porb->orbital_advection_defined && std::strcmp(COORDINATE_SYSTEM, "cartesian") != 0)
...
void turner(FieldDiffusion *pfdif, MeshBlock *pmb, const AthenaArray<Real> &w,
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      for(int i=is; i<=ie; i++) {
        Real dens = w(IDN,k,j,i) * dens_scale;                  // in cgs unit
        Real temp = w(IEN,k,j,i) / w(IDN,k,j,i) * temp_scale;   // in cgs unit
        ...
        for(int ic=int(ilo); ic<=int(ilo)+1; ic++){
          ...
          for(int jc=int(jlo); jc<=int(jlo)+1; jc++){
            Real facj=1-abs(jc-jlo);
            for(int kc=int(klo); kc<=int(klo)+1; kc++){
              Real fack=1-abs(kc-klo);
              for(int hc=int(hlo); hc<=int(hlo)+1; hc++){
